...
sorted_detectors = sorted(
    chi2_detectors,
    key=lambda d: d["arrival"]
)

first10 = sorted_detectors[:10]

d = first10[0]

seed_pos = np.mean(
    [d["pos"] for d in first10],
    axis=0
)

seed_error = np.linalg.norm(
    seed_pos - source_pos
)
...
print(
    "dx =", seed_pos[0] - source_pos[0]
)

print(
    "dy =", seed_pos[1] - source_pos[1]
)

print(
    "dz =", seed_pos[2] - source_pos[2]
)

print()
print("TRUE :", source_pos)
print("SEED :", seed_pos)
print(f"SEED ERROR = {seed_error:.1f} m")
print()

print(len(chi2_detectors))
...
x_scan = np.linspace(seed_x - 150,
                     seed_x + 150,
                     50)

y_scan = np.linspace(seed_y - 150,
                     seed_y + 150,
                     50)
...
print("scan width x =", x_scan[-1] - x_scan[0])
print("scan width y =", y_scan[-1] - y_scan[0])
...
print(
    "NaNs in map =",
    np.sum(~np.isfinite(chi2_map))
)
print("chi2 min =", np.min(chi2_map))
print("chi2 max =", np.max(chi2_map))
print("delta chi2 max =", np.max(delta_chi2_map))

best_index = np.unravel_index(
    np.argmin(delta_chi2_map),
    delta_chi2_map.shape
)

best_y = y_scan[best_index[0]]
best_x = x_scan[best_index[1]]

print("best delta =", -np.min(chi2_map))
...
true_ix = np.argmin(np.abs(x_scan - x0))
true_iy = np.argmin(np.abs(y_scan - y0))

print("true_ix =", true_ix)
print("true_iy =", true_iy)

print("true scan x =", x_scan[true_ix])
print("true scan y =", y_scan[true_iy])

print("true x =", x0)
print("true y =", y0)

print(
    "distance true point to nearest grid point =",
    np.sqrt(
        (x_scan[true_ix] - x0)**2 +
        (y_scan[true_iy] - y0)**2
    )
...
print()
print("TRUE :", source_pos)
print("BEST :", [best_x, best_y, z0])
print(
    "ERROR =",
    np.linalg.norm(
        np.array([best_x,best_y,z0]) - source_pos
    )
)

fit_error = np.linalg.norm(
    np.array([best_x,best_y,z0]) - source_pos
)
fit_errors.append(fit_error)

print()
print("MEAN SEED ERROR =", np.mean(seed_errors))
print("MEAN FIT ERROR =", np.mean(fit_errors))

print("Minimum Δχ² =", np.min(delta_chi2_map))
...
print("x range =", x_scan[0], x_scan[-1])
print("y range =", y_scan[0], y_scan[-1])
